...
public class BookController {
	...
	@Autowired
	private BookTopicService bTServise;
	
	

//	і в підкатегорії треба подивтись до сейву
//	не працюють фільтри по ціні і немає ще серчів усіх
//	помилка валідації
	...
	public String save(@ModelAttribute("book") @Valid BookForm book, BindingResult br, Model model, SessionStatus status, @PageableDefault Pageable pageable, @ModelAttribute("filter") BookFilter filter){
		if(br.hasErrors()){
			model.addAttribute("page", bService.findAll(filter,pageable));
			model.addAttribute("bookaditions", baddService.findAll());
			model.addAttribute("bookautors", baService.findAll());
